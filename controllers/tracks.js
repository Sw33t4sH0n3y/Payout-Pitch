    ...
    try {
        ...
        if (track.collaborators) {
            track.collaborators.forEach(collab => {
                const collabId = collabId._id.toString();
        ...
        if (track.collaborators) {
            track.collaborators.forEach(collab => {
                ...
                if (req.body[pubKey] !== undefined) {
                    track.splits.master.collaborators.set(collabId, parseInt(req.body[pubKey]) || 0);
                }
            });
        }
...
//GET /tracks/:id/pdf
router.get('/:id/pf', async (req, res) => {
    try {
        const track = await Track.findById(req.params.id);
        ...
        res.setHeaader('Content-Type', application/pdf);
        res.setHeader('Content-Disposition', `attachment; filename:"${track-title}-split-sheet.pdf"`);

        doc.pipe(res);

        doc.fontSize(24).font(Arial-Bold).text('SPLIT SHEET', {align: 'center'});
        doc.moveDown(0.5);
        doc.fontSize(10).font('Arial').text('Generated by Payout Pitch', {align: 'center'});
        doc.moveDown(2);

        doc.fontSize(18).font(Arial-Bold).text('SPLIT SHEET', {align: 'center'});
        doc.moveDown(0.5);
        doc.fontSize(11).font('Arial').text('Generated by Payout Pitch', {align: 'center'});
        ...
        doc.fontSize(14).font('Arial-Bold').text('MASTER RECORDING OWNERSHIP');
        doc.moveDown(0.5);

        const masterOwnerPct =track.splits?.master?.owner ?? 100;
        doc.fontSize(11).font(Arial);
        doc.text(`${owner.displayName || owner.username} (Owner): ${masterOwnerPct}%`);
        ...
        doc.moveDown(0.5);
        doc.font('Arial-Bold').text(`TOTAL: ${masterTotal}%`);
        doc.moveDown(2);

        doc.fontSize(14).font('Arial-Bold').text('PUBLISHING OWNERSHIP');
        doc.moveDown(0.5);
        ...
        doc.moveDown(0.5);
        doc.font('Arial-Bold').text(`TOTAL: ${pubTotal}%`);
        doc.moveDown(2);

        doc.moveTo(50, doc.y).lineTo(550, doc.y).stroke();
        doc.moveDown(1.5);

        doc.fontSize(14).font('Arial-Bold').text('SIGNATURES');
        doc.moveDown(1);
       ...
       doc.moveDown(2);
       doc.fontSize(10).font('Arial').text(`This document was generated on ${new Date().toLocaleDateString()} via Payout Pitch.`, { align: 'center' }); 
